    ...
    do
        ...
        if $QUIET ; then   
            if [ ! -f "$script_path/config/config.$mcu" ]; then
                error_exit "${1^} No config file for $mcu, \nDon't use quiet mode on first fimware update"
            fi  
        else 
        ...
        if prompt "No errors? Press [Y] to flash $mcu" ; then
            ...
            for command in "${commands[@]}"; do
                ...
                if [[ "$command" =~ [[:space:]]*enter_bootloader ]]; then
                    # Execute call_bootloader directly
                    $command
                else
...
# Define a function to enter bootloader mode
# Usage  : enter_bootloader [type:usb|serial|can] [serial|canuuid] [baudrate]
function enter_bootloader() {
    ...
    case "$type" in
        usb)
            echo "Entering USB bootloader mode for $serial"
            cd ~/klipper/scripts
            python3 -c "import flash_usb as u; u.enter_bootloader('$serial')"; sleep 2
            ;;
        serial)
            echo "Entering serial bootloader mode for $serial"
            baudrate=${baudrate:-250000}
            python3 -c "import serial; with serial.Serial($serial, $baudrate, timeout=1) as ser:
                  ser.write(b'~\x1c Request Serial Bootloader!! ~')"; sleep 2
            ;;
        can)
            echo "Entering CAN bootloader mode for $serial"
            if [[ -f ~/katapult/scripts/flashtool.py ]]; then
              ~/katapult/scripts/flashtool.py -r -u $serial ; sleep 2
            else
